    ...
    class mdirectorWidget extends WP_Widget
    {
        ...
        public function mdirectorWidget()
        {
            $widget_ops = array(
                ...
                'description' => __('Formulario de suscripción',
                    'mdirector-newsletter')
            );
            $this->WP_Widget('mdirectorWidget', 'MDirector Widget',
                $widget_ops);
        }
        ...
        public function form($instance)
        {
            ...
            ?>
            <p><label
                    for="<?php echo $this->get_field_id('title'); ?>"><?php echo __('Título',
                        'mdirector-newsletter'); ?>: <input class="widefat"
            ...
            <p>
                <label for="<?php echo $this->get_field_id('description'); ?>">
                    <?php echo __('Texto explicativo',
                        'mdirector-newsletter'); ?>:
                    <textarea class="widefat"
                        id="<?php echo $this->get_field_id('description'); ?>"
                        name="<?php echo $this->get_field_name('description'); ?>"><?php echo attribute_escape($description); ?></textarea>
                </label>
            </p>
            <?php
        }
        ...
        public function widget($args, $instance)
        {
            ...
            if (empty($settings['frequency_daily'])
                && empty($settings['frequency_weekly'])) {
                return false;
            }
            ...
            if ($mdirector_active === 'yes') {
                if ($settings['frequency_daily'] === 'yes'
                    ...
                    $select_frequency .= '<option value="daily">'
                        . __('Recibir newsletter diaria',
                            'mdirector-newsletter') . '</option>';
                    $select_frequency .= '<option value="weekly">'
                        . __('Recibir newsletter semanal',
                            'mdirector-newsletter') . '</option>';
                    $select_frequency .= '</select>';
                    $select_frequency .= '</div>';
                } else if ($settings['frequency_daily'] === 'yes') {
                    $select_frequency =
                        '<input value="daily" name="md_frequency" type="hidden">';
                } else if ($settings['frequency_weekly'] === 'yes') {
                    $select_frequency =
                        '<input value="weekly" name="md_frequency" type="hidden">';
                }
                if ($settings['api'] && $settings['secret']) {
                    ...
                    echo '
                            ...
                            <a href="' . $md_privacy_link
                        . '" target="_blank" class="md__newsletter--accept">'
                        . $accept . '</a>
                            </label>
                            </p>';
                    ...
                    echo '
                            <div class="md__newsletter--area__button">
		    				    <button class="md_newsletter--button">'
                        . __('Suscribirme', 'mdirector-newsletter') . '</button>
		    			    </div>
		    	    </form>
		    	    <div class="md_ajax_loader md_widget"><img src="'
                        . MDIRECTOR_NEWSLETTER_PLUGIN_URL
                        . 'assets/ajax-loader.gif' . '"></div>';
                }
            }
        }
    }
